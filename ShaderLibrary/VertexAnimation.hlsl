...
float4 WindAnimation(float4 nv1)
{
    ...
    float _anchored = sin((_Time.y + nv.x + positionWS.x + positionWS.z) * _Speed + nv.y * _Amount) * _Distance * (_DistanceFromOrigin / 3);
    float _unanchored = sin((_Time.y + nv.z + positionWS.x + positionWS.z) * _Speed + nv.y * _Amount) * _Distance;
