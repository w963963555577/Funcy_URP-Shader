...
half4 MixGlobalFog(half4 color, half3 positionWS)
{
    half fogArea = min(1.0, abs(length(positionWS - _WorldPoint.xyz) * _WorldPoint.w));
    half center = smoothstep(0.4, 1.1, 1.0 - fogArea);
    half g1 = SAMPLE_TEXTURE2D(_GlobalFogNoise, sampler_GlobalFogNoise, (positionWS.xz + _Time.y * 2.000 * half2(-0.317, -0.899641)) * _GlobalFogNoiseScale).r;
    half g2 = SAMPLE_TEXTURE2D(_GlobalFogNoise, sampler_GlobalFogNoise, (positionWS.xz + _Time.y * 1.513 * half2(0.217, -0.699641)) * _GlobalFogNoiseScale).r;
    half gray = max(g2, g1);
