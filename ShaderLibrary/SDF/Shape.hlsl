...
float sdCone1(in float3 p, in float2 c, float h)
{
...
float sdCone2(float3 p, float2 c, float h)
{
    float q = length(p.xz);
    return max(dot(c.xy, float2(q, p.y)), -h - p.y);
}

float sdCone3(float3 p, float2 c)
{
...
float sdCappedCone1(float3 p, float h, float r1, float r2)
{
    float2 q = float2(length(p.xz), p.y);
...
float sdCappedCone2(float3 p, float3 a, float3 b, float ra, float rb)
{
    float rba = rb - ra;
...
float sdRoundCone1(float3 p, float r1, float r2, float h)
{
    float2 q = float2(length(p.xz), p.y);
...
float sdRoundCone2(float3 p, float3 a, float3 b, float r1, float r2)
{
...
float sdOctahedron1(float3 p, float s)
{
    p = abs(p);
...
float sdOctahedron2(float3 p, float s)
{
    p = abs(p);
