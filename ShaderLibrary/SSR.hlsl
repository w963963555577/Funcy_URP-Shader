...
float3 SSRRayMarch(float4 clipPosition, float3 worldPos, half3 R)
{
    float4 startClipPos;
...
float3 GetSSRUVZ(float4 clipPosition, float3 worldPos, half NoV, half3 R, float2 screenUV)
{
    #if defined(UNITY_SINGLE_PASS_STEREO)
        half ssrWeight = 1;
    ...
    if(ssrWeight > 0.005)
    {
        float3 uvz = SSRRayMarch(clipPosition, worldPos, R);
        uvz.z *= ssrWeight;
        return uvz;
    }
