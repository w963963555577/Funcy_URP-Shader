Shader "ZDShader/LWRP/Character(DoubleFace)"
{
    Properties
    {
        _diffuse ("BaseColor", 2D) = "white" { }
        _diffuseDetal ("DetalColor", 2D) = "white" { }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _diffuseDetal;
            CBUFFER_START(UnityPerMaterial)
            half4 _diffuse_ST;
            half4 _diffuseDetal_ST;
            half4 _SelfMask_ST;
        ...
        Pass
        {
            ...
            half4 LitPassFragment(Varyings i): SV_Target
            {
                ...
                float specStep = 2.0;
                float specularArea = saturate(floor(pow(max(0, dot(i.normalWS, halfDirection)), exp2(lerp(1, 11, (_Gloss_var * glossMask)))) * specStep) / (specStep - 1));
        ...
        Pass
        {
            ...
            // to render objects.
            Name "Detal"
            Tags { "LightMode" = "UniversalAdditional" }
            ...
            half4 LitPassFragment(Varyings i): SV_Target
            {
                ...
                float4 _diffuse_var = tex2D(_diffuse, i.uv01.xy);
                float4 _diffuseDetal_var = tex2D(_diffuseDetal, i.uv01.xy);
                ...
                float shadowTotal = saturate(shadowArea0 + shadowArea1 + shadowAreaElse) * (1.0 - PBRShadowArea);
                float3 diffuseColor = lerp(_diffuseDetal_var.rgb, _diffuseDetal_var.rgb * saturate(shadowArea0 * _ShadowColor0.rgb + shadowArea1 * _ShadowColor1.rgb + shadowAreaElse * _ShadowColorElse.rgb), shadowTotal);
    ...
    // same properties.
    CustomEditor "UnityEditor.Rendering.Funcy.LWRP.ShaderGUI.ZDCharacter_DoubleFace"
}