	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 _Vector0 = float2(0.9,1);
				float smoothstepResult46 = smoothstep( _Vector0.x , _Vector0.y , ( 1.0 - abs( sin( ( length( ( ( IN.ase_texcoord2.xy - float2( 0.5,0.5 ) ) * float2( 2,2 ) ) ) - (-0.1 + (IN.ase_texcoord2.zw.x - 0.0) * (1.5 - -0.1) / (1.0 - 0.0)) ) ) ) ));
				
				float Alpha = saturate( pow( ( IN.ase_color.a * smoothstepResult46 ) , IN.ase_texcoord2.zw.y ) );
				float AlphaClipThreshold = 0.5;
