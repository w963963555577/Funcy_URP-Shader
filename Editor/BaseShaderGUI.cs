...
namespace UnityEditor
{
    ...
    public abstract class BaseShaderGUI : ShaderGUI
    {
        ...
        public virtual void DrawAdvancedOptions(Material material)
        {
            ...
            ChangeCheckArea_Float(material, zWriteProp, "ZWrite");
            ChangeCheckArea_Float(material, zTestProp, "ZTest");


            EditorGUI.BeginChangeCheck();
            int valueQueue = EditorGUILayout.IntField("Render Queue", material.renderQueue);

            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(material, material.GetInstanceID() + "Queue");
                material.renderQueue = valueQueue;
            }

        }

        public void ChangeCheckArea_Float(Material material, MaterialProperty materialProperty, string appearName)
        {
            EditorGUI.BeginChangeCheck();
            float value = materialProperty.floatValue;
            if (EditorGUI.EndChangeCheck())
            {
                Undo.RecordObject(material, material.GetInstanceID() + materialProperty.name);
                materialProperty.floatValue = value;
            }
            materialEditor.ShaderProperty(materialProperty, appearName);
